...
This creates the six required tables and indexes:
- `mplads_ingest_batch`
...
Existing deployments must rerun the idempotent initialiser before using locality evidence. The location table starts empty; it does not generate coordinates or alter source records.
