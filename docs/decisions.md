...
- **Problem:** The user requested consistent branding and geographic comparison of the two works. Administrative names alone cannot establish exact coordinates, and illustrative design analytics are not evidence.
- **Alternatives:** Static map artwork, externally embedded maps, hand-written projection/panning, React-specific map wrappers or geocoding administrative names. These either do not support precise interactive inspection, add unnecessary integration, or fabricate precision.
- **Selected approach:** Reuse the existing candidate location contract and summary endpoint. Dynamically load Leaflet only for the comparison tool; bundle an attributed geoBoundaries outline and make street tiles optional. Render only valid `VERIFIED_COORDINATES`; distinct labels remain selectable at coincident coordinates. Source clicks use a read-only Next.js route which validates session and source membership in the selected candidate. Display all available source fields and preserve missing values. The map's straight-line distance is display-only and never alters detector evidence.
- **Library selection:** Leaflet 1.9.4 and `@types/leaflet` 1.9.21 for established geographic projection, bounds, scale, accessible markers and tile handling. Direct integration avoids a second React wrapper. Existing Lucide and native dynamic imports are reused. The map shell renders with the page, while Leaflet loads inside a browser effect; an overlay preserves the layout during loading. Local installed Next.js documentation and [Leaflet's reference](https://leafletjs.com/reference.html) informed integration. Context7 and Playwright MCP tools were unavailable in this session; repository Playwright CLI is used for QA, not represented as MCP execution.
- **Trade-offs:** The third-party outline is reference cartography, not an official legal boundary; public deployment requires administrative review. Optional [OSM tiles](https://operations.osmfoundation.org/policies/tiles/) require connectivity and expose browser IP/requested tile area to the provider. No work payload is sent. Source-detail clicks currently refetch the existing full candidate server-side before selecting its record; no new backend endpoint or cache is added.
- **Performance impact:** Approximately 90 KB local GeoJSON plus a deferred map bundle on evidence pages. Only the selected pair creates markers. Stale requests are aborted, and detail fetches have a ten-second browser timeout. Default maps make no third-party request.
- **Maintainability impact:** Shared Brand and stylesheet preserve existing route structure. Location validation, map mechanics and source detail rendering are separate modules; asset origin/licence is recorded alongside the GeoJSON. PNG source is unchanged and framed with CSS. The fifth design's state/trend/financial values are intentionally not imported.
- **Security impact:** Credentials remain server-side. Source requests require signed sessions, bounded identifiers and candidate membership, with private/no-store responses. React escapes displayed fields; marker HTML contains constant A/B labels only. No review mutation, detector change, schema change, geocoding or data fabrication.
- **Affected files:** `frontend/app/brand.tsx`, layout, shared shell/login/CSS, Command Centre page and `review-overview.tsx`, candidate detail page, `location-comparison.tsx`, `location-map-canvas.tsx`, `location-types.ts`, `[id]/source/route.ts`, investigations types, public brand/maps assets, frontend manifest/lock, `e2e/location-comparison.spec.ts` and design/architecture/flow/PRD/techstack/source/feature/log documentation.

## 2026-09-25: Adapt the supplied Stitch designs to the working review flow

- **Decision:** Reimplement the supplied sign-in and evidence-workspace designs using the current Next.js pages, a shared dark header/light sidebar and a responsive evidence/review layout.
- **Problem:** The user requested the new UI, but the exported HTML includes illustrative records, dead controls and unsupported identity, financial and legal claims.
- **Alternatives:** Embed the standalone exported HTML, copy every mock control, or change only colours. Native React components preserve the actual authenticated workflow and allow the structural evidence/review design to work across screen sizes.
- **Selected approach:** Preserve server rendering, existing handlers and the user's in-progress locality changes. Keep real data in the evidence summary and source comparison, and place the review form/history alongside evidence on large screens. Provide working password visibility and pending submission feedback. Document implementation in `docs/design.md`.
- **Library selection:** Add `lucide-react` for consistently sized navigation and action icons; retain the existing Geist fonts, CSS and platform form controls. No UI framework, chart library or runtime CDN is required.
- **Trade-offs:** Visual structure follows the references, but unsupported controls and illustrative claims are excluded. Production identity integration and source-data limitations remain unchanged.
- **Performance impact:** Pages remain server-rendered, with small client components for password visibility, submission state and the existing export. No additional data requests or large browser datasets are introduced.
