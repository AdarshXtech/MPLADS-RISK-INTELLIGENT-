...
from backend.init_db import ALL_DDL, init_db, main


def test_init_db_includes_reviewed_locations_after_source_tables():
    names = [name for name, _ in ALL_DDL]
    assert names.index("staging") < names.index("locations")
    assert (
        "CREATE TABLE IF NOT EXISTS mplads_work_location" in dict(ALL_DDL)["locations"]
    )
