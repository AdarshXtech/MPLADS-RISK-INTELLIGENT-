...
import { ArrowLeft, Files, MapPin, ScanSearch, ShieldAlert } from "lucide-react";
...
export default async function CandidatePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  try { item = await getCandidate(id); } catch (error) {
    ...
    return <QueueShell username={username} connected={false}><main className="page-content" id="main-content" tabIndex={-1}><section className="error-panel" role="alert"><h1>Candidate unavailable</h1><p>The review service could not load this evidence. Return to the queue and try again.</p><Link className="retry-link" href="/investigation-queue">Return to queue</Link></section></main></QueueShell>;
  }
  ...
  const locality = item.evidence.locality_evidence as Record<string, unknown> | undefined;
  const distance = item.distance_metres === null ? "Not available" : `${item.distance_metres.toFixed(1)} m`;
  const supportingSignals = item.evidence.supporting_signals as Array<{ label?: string; value?: string }> | undefined;
  return <QueueShell username={username}><main className="page-content" id="main-content" tabIndex={-1}>
    <Link className="back-link" href="/investigation-queue"><ArrowLeft size={16} aria-hidden="true" />Back to Investigation Queue</Link>
    ...
    <dl className="case-summary" aria-label="Candidate screening summary">
      <div><dt><ShieldAlert size={16} aria-hidden="true" />Severity</dt><dd>{item.severity}</dd><p>Requires administrative verification</p></div>
      <div><dt><ScanSearch size={16} aria-hidden="true" />Match confidence</dt><dd>{Math.round(item.confidence * 100)}%</dd><p>Matching confidence, not probability of misuse</p></div>
      <div><dt><Files size={16} aria-hidden="true" />Source records</dt><dd>{item.group_size}</dd><p>Provenance retained for comparison</p></div>
      <div><dt><MapPin size={16} aria-hidden="true" />Calculated distance</dt><dd>{distance}</dd><p>{item.locality_level.replaceAll("_", " ").toLowerCase()}</p></div>
    </dl>
    <div className="case-layout"><div className="case-evidence">
    ...
    <section className="panel"><div className="panel-header"><div><h2 className="panel-title">Locality and supporting evidence</h2><p className="panel-description">Location evidence narrows screening. It cannot by itself establish a potential duplicate.</p></div></div><LocationComparison candidateId={item.result_id} sources={item.source_records.map(({ source_sha256, parser_version, record_number, work_id, location }) => ({ source_sha256, parser_version, record_number, work_id, location }))} /><dl className="matched-grid"><div><dt>Locality comparison</dt><dd>{String(locality?.label ?? "Exact context version 1")}</dd></div><div><dt>Coordinate availability</dt><dd>{String((item.evidence.location_summary as Record<string, unknown> | undefined)?.coordinate_availability ?? "Not supplied")}</dd></div><div><dt>Calculated distance</dt><dd>{distance}</dd></div><div><dt>Matched administrative fields</dt><dd>{Object.entries((item.evidence.matched_administrative_fields as Record<string, unknown> | undefined) ?? {}).map(([key, value]) => `${key}: ${String(value)}`).join(", ") || "Not supplied"}</dd></div><div><dt>Supporting signals</dt><dd>{supportingSignals?.map((signal) => `${signal.label ?? "Signal"}: ${signal.value ?? ""}`).join("; ") || "Not applicable to exact-context version 1"}</dd></div></dl></section>
    </div><aside className="case-review" aria-label="Reviewer decision and history">
    <section className="panel"><div className="panel-header"><div><p className="eyebrow">Reviewer workspace</p><h2 className="panel-title">Record review action</h2><p className="panel-description">Each save creates a new audit event. Earlier entries cannot be overwritten.</p></div></div><form className="review-form" action={action}>
      ...
      <SubmitButton pendingLabel="Saving review...">Save review action</SubmitButton>
    </form></section>
