...
import { getCandidates, type LocationStatus, type Sort, type Status } from "@/lib/investigations";
...
const statuses: Array<{ value: Status | ""; label: string }> = [
  ...
  { value: "UNDER_REVIEW", label: "Under review" }, { value: "VERIFICATION_REQUESTED", label: "Verification requested" },
  { value: "RESOLVED", label: "Resolved" }, { value: "DISMISSED", label: "Dismissed" },
];
const label = (status: Status) => status.toLowerCase().replaceAll("_", " ");
const localityLabel = (value: string) => value.replaceAll("_", " ");
const sorts: Array<{ value: Sort; label: string }> = [
  { value: "group_smallest", label: "Smallest groups first" }, { value: "group_largest", label: "Largest groups first" },
  { value: "state", label: "State A to Z" }, { value: "recently_reviewed", label: "Most recently reviewed" },
];
type Params = { page?: string; query?: string; state?: string; status?: string; sort?: string; locality?: string; location_status?: string };
const isSort = (value: string | undefined): value is Sort => sorts.some((sort) => sort.value === value);
const distance = (metres: number | null) => metres === null ? null : metres < 1000 ? `${metres.toFixed(0)} m` : `${(metres / 1000).toFixed(2)} km`;
...
export default async function InvestigationQueuePage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const parameters = new URLSearchParams({ page: String(page), page_size: "20", sort: selectedSort });
  ...
  return <QueueShell username={username}><main className="page-content" id="main-content" tabIndex={-1}>
    ...
    <form className="filter-bar" method="get" role="search">
      <div className="filter-heading"><ListFilter size={17} aria-hidden="true" /><span>Candidate filters</span></div>
      ...
      <div><label htmlFor="locality">Locality comparison</label><select id="locality" name="locality" defaultValue={supplied.locality ?? ""}><option value="">All comparison levels</option>{result.localities.map((locality) => <option key={locality} value={locality}>{localityLabel(locality)}</option>)}</select></div>
      <div><label htmlFor="location_status">Location status</label><select id="location_status" name="location_status" defaultValue={supplied.location_status ?? ""}><option value="">All location statuses</option>{result.location_statuses.map((value: LocationStatus) => <option key={value} value={value}>{localityLabel(value)}</option>)}</select></div>
      ...
      <button type="submit">Apply filters</button><Link className="secondary-link" href="/investigation-queue">Clear</Link>
    </form>
    ...
    {result.items.length === 0 ? <section className="empty-panel"><h2>No candidates match these filters</h2><p>Change or clear the filters to see other review candidates.</p></section> : <section className="panel" aria-labelledby="candidate-title">
      ...
      <div className="table-wrap queue-table-wrap"><table className="source-table queue-table"><caption className="sr-only">Potential duplicate candidates</caption><thead><tr><th scope="col">Status</th><th scope="col">Matched work</th><th scope="col">Locality and agency</th><th scope="col">Location status</th><th scope="col">Records</th><th scope="col">Action</th></tr></thead><tbody>{result.items.map((item) => <tr key={item.result_id}><td><span className={`status-chip status-${item.status.toLowerCase()}`}>{label(item.status)}</span></td><td><strong>{item.work_description}</strong><p className="cell-note">{item.work_ids.slice(0, 3).join(", ")}{item.work_ids.length > 3 ? ` and ${item.work_ids.length - 3} more` : ""}</p></td><td>{item.state}<p className="cell-note">{localityLabel(item.locality_level)} · {item.constituency} · {item.ida}</p></td><td><span className="location-chip">{localityLabel(item.location_statuses.join(", "))}</span>{distance(item.distance_metres) && <p className="cell-note">{distance(item.distance_metres)}</p>}</td><td>{integer.format(item.group_size)}</td><td><Link className="row-action" href={`/investigation-queue/${item.result_id}`}>Review evidence</Link></td></tr>)}</tbody></table></div>
      <div className="candidate-cards">{result.items.map((item) => <article className="candidate-card" key={`card-${item.result_id}`}><div className="review-row"><span className={`status-chip status-${item.status.toLowerCase()}`}>{label(item.status)}</span><strong>{item.group_size} records</strong></div><h3>{item.work_description}</h3><p>{item.state}, {item.constituency}</p><p className="cell-note">{localityLabel(item.locality_level)} · {localityLabel(item.location_statuses.join(", "))}</p>{distance(item.distance_metres) && <p className="cell-note">Distance: {distance(item.distance_metres)}</p>}<p className="cell-note">{item.ida}</p><Link className="row-action" href={`/investigation-queue/${item.result_id}`}>Review evidence</Link></article>)}</div>
    </section>}
