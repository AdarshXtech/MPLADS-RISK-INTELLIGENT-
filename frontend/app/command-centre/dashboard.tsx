...
type OverviewResult =
  | { status: "ready"; data: DataOverview; summary: InvestigationSummary | null }
  | { status: "error" };
...
async function getOverview(view: "command-centre" | "data-quality"): Promise<OverviewResult> {
  await connection();
      ...
      <div className="analysis-grid command-analysis">
        <section className="panel" id="data-quality" aria-labelledby="sources-title">
          <div className="panel-header">
            ...
            <span className="count-badge">{integer.format(data.source_batches)} {data.source_batches === 1 ? "source" : "sources"}</span>
          </div>
...
export async function ReviewerDashboard({ view }: { view: "command-centre" | "data-quality" }) {
  const username = await requireReviewer();
  const result = await getOverview(view);
  const connected = result.status === "ready";

  return (
    <QueueShell username={username} active={view} connected={connected}>
          <main className="page-content" id="main-content" tabIndex={-1}>
            <div className="page-heading-row">
              <div>
                <p className="eyebrow">{view === "data-quality" ? "Source validation" : "Data readiness"}</p>
                <h1>{view === "data-quality" ? "Data Quality" : "Risk Command Centre"}</h1>
                <p className="page-intro">{view === "data-quality" ? "Ingestion counts, validation issues and source coverage for the reports available to this project." : "Verified ingestion status for the MPLADS reports currently available to this project."}</p>
              </div>
            ...
            {result.status === "ready" && view === "command-centre" && result.summary && <ReviewOverview summary={result.summary} />}
            ...
            {result.status === "error" ? (
              <section className="error-panel" id="data-quality" role="alert" aria-labelledby="service-error-title">
                ...
                <p>The {view === "data-quality" ? "Data Quality page" : "Command Centre"} could not read the verified ingestion summary. Confirm that FastAPI and PostgreSQL are running, then retry.</p>
